...
class GPSNode(Node):
    def __init__(self):
        ...
        # Publishers (NO STRING → FIXED DDS ISSUE)
    ...
    def _read_loop(self):
        while self.running:
            try:
                ...
                # 🔥 Only parse useful messages
                if "$GNGGA" not in line and "$GNRMC" not in line:
                    continue

                msg = pynmea2.parse(line)

                # -------- GGA --------
                if isinstance(msg, pynmea2.GGA) and msg.gps_qual and int(msg.gps_qual) > 0:
                    with self.lock:
    ...
    def _publish(self):
        ...
        vel_msg.twist.angular.z = math.radians(heading)
